import React, { useEffect, useMemo, useState } from 'react';
import { Container, Row, Col, Card, Button, Table, Badge, Modal, Form, Spinner, Alert } from 'react-bootstrap';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faBook, faEye, faPen, faTrash, faUser, faArrowLeft } from '@fortawesome/free-solid-svg-icons';
import { useNavigate, useParams } from 'react-router-dom';
import { apiClient } from '../../../api/client';
import './AdminFormations.css';

const emptySession = { date_start: '', date_end: '', nbPlace: '', intervenantId: '' };

const AdminFormationSessions = () => {
    const navigate = useNavigate();
    const { id: formationId } = useParams();
    const [showAddModal, setShowAddModal] = useState(false);
    const [showEditModal, setShowEditModal] = useState(false);
    const [selectedSession, setSelectedSession] = useState(null);
    const [sessions, setSessions] = useState([]);
    const [seances, setSeances] = useState([]);
    const [formation, setFormation] = useState(null);
    const [intervenants, setIntervenants] = useState([]);
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);
    const [success, setSuccess] = useState(null);
    const [createForm, setCreateForm] = useState(emptySession);
    const [editForm, setEditForm] = useState(emptySession);

    useEffect(() => {
        fetchFormation();
        fetchSessions();
        fetchSeances();
        fetchIntervenants();
    }, [formationId]);

    const fetchFormation = async () => {
        try {
            const data = await apiClient.get(`/formation/${formationId}`);
            setFormation(data);
        } catch (err) {
            setError("Impossible de charger les informations de la formation.");
        }
    };

    const fetchSessions = async () => {
        setIsLoading(true);
        setError(null);
        try {
            const data = await apiClient.get('/session');
            const filtered = Array.isArray(data)
                ? data.filter((s) => s?.formation?.id === Number(formationId))
                : [];

            const sorted = [...filtered].sort((a, b) => {
                const dateA = a?.date_start ? new Date(a.date_start) : null;
                const dateB = b?.date_start ? new Date(b.date_start) : null;
                if (!dateA && !dateB) return 0;
                if (!dateA) return 1;
                if (!dateB) return -1;
                return dateA - dateB;
            });

            setSessions(sorted);
        } catch (err) {
            setError("Impossible de charger les sessions.");
        } finally {
            setIsLoading(false);
        }
    };

    const fetchSeances = async () => {
        try {
            const data = await apiClient.get('/seance');
            setSeances(Array.isArray(data) ? data : []);
        } catch (err) {
        }
    };

    const fetchIntervenants = async () => {
        try {
            const data = await apiClient.get('/intervenant');
            setIntervenants(Array.isArray(data) ? data : []);
        } catch (err) {
        }
    };

    const seancesBySession = useMemo(() => {
        return seances.reduce((acc, seance) => {
            const sessionId = seance.session?.id;
            if (!sessionId) return acc;
            acc[sessionId] = (acc[sessionId] || 0) + 1;
            return acc;
        }, {});
    }, [seances]);

    const totalSeances = useMemo(() => {
        return sessions.reduce((total, session) => total + (seancesBySession[session.id] || 0), 0);
    }, [sessions, seancesBySession]);

    const totalPlaces = useMemo(() => {
        return sessions.reduce((total, session) => total + Number(session.nbPlace || 0), 0);
    }, [sessions]);

    const handleShowAdd = () => setShowAddModal(true);
    const handleCloseAdd = () => {
        setShowAddModal(false);
        setCreateForm(emptySession);
    };

    const handleShowEdit = (session) => {
        setSelectedSession(session);
        setEditForm({ ...emptySession, ...session, intervenantId: session.intervenant?.id || '' });
        setShowEditModal(true);
    };
    const handleCloseEdit = () => {
        setShowEditModal(false);
        setSelectedSession(null);
        setEditForm(emptySession);
    };

    const handleChange = (setter) => (event) => {
        const { name, value } = event.target;
        setter((prev) => ({ ...prev, [name]: value }));
    };

    const handleCreateSession = async (event) => {
        event.preventDefault();
        
        if (createForm.date_start && createForm.date_end && createForm.date_start >= createForm.date_end) {
            setError("La date de début doit être inférieure à la date de fin.");
            return;
        }
        
        try {
            const payload = {
                ...createForm,
                formation: { id: Number(formationId) },
                nbPlace: createForm.nbPlace,
            };
            if (createForm.intervenantId) {
                payload.intervenant = { id: Number(createForm.intervenantId) };
            }
            await apiClient.post('/session', payload);
            await fetchSessions();
            handleCloseAdd();
        } catch (err) {
            setError("La création de la session a échoué.");
        }
    };

    const handleUpdateSession = async (event) => {
        event.preventDefault();
        if (!selectedSession) return;
        
        if (editForm.date_start && editForm.date_end && editForm.date_start >= editForm.date_end) {
            setError("La date de début doit être inférieure à la date de fin.");
            return;
        }
        
        try {
            const payload = {
                ...editForm,
                formation: { id: Number(formationId) },
                nbPlace: editForm.nbPlace,
            };
            if (editForm.intervenantId) {
                payload.intervenant = { id: Number(editForm.intervenantId) };
            }
            await apiClient.put(`/session/${selectedSession.id}`, payload);
            await fetchSessions();
            handleCloseEdit();
        } catch (err) {
            setError("La mise à jour de la session a échoué.");
        }
    };

    const handleDeleteSession = async (id) => {
        if (!window.confirm("Êtes-vous sûr de vouloir supprimer cette session ?")) {
            return;
        }
        try {
            await apiClient.delete(`/session/${id}`);
            setSessions((prev) => prev.filter((s) => s.id !== id));
            setSuccess("Session supprimée avec succès.");
            setError(null);
            setTimeout(() => setSuccess(null), 3000);
        } catch (err) {
            setError("Impossible de supprimer cette session.");
            setSuccess(null);
        }
    };

    return (
        <Container fluid className="admin-formations p-0">
            <div className="mb-4">
                <div className="mb-3">
                    <Button variant="link" className="p-0 text-dark text-decoration-none d-inline-flex align-items-center" onClick={() => navigate('/admin/formations')}>
                        <FontAwesomeIcon icon={faArrowLeft} size="lg" />
                        <span className="ms-2 fw-bold">Retour</span>
                    </Button>
                </div>
                <div>
                    <h2 className="fw-bold text-dark-teal mb-0">
                        <div className="d-inline-flex align-items-center justify-content-center bg-dark-teal text-white rounded-2 me-3" style={{ width: 40, height: 40 }}>
                            <FontAwesomeIcon icon={faBook} size="sm" />
                        </div>
                        {formation ? `${formation.nom} – Sessions` : 'Sessions de la formation'}
                    </h2>
                    <p className="text-muted ms-5 ps-4 mt-2">Liste des sessions de la formation</p>
                </div>
            </div>

            <Row className="g-4 mb-5">
                <Col md={4}>
                    <Card className="border-0 shadow-sm rounded-4 p-3">
                        <div className="d-flex justify-content-between align-items-center">
                            <div>
                                <h6 className="text-muted mb-2 fw-bold">Nombre de sessions</h6>
                                <h2 className="text-dark-teal fw-bold mb-0">{sessions.length}</h2>
                            </div>
                            <div className="stat-icon-square bg-dark-teal text-white rounded-3 d-flex align-items-center justify-content-center">
                                <FontAwesomeIcon icon={faBook} />
                            </div>
                        </div>
                    </Card>
                </Col>
                <Col md={4}>
                    <Card className="border-0 shadow-sm rounded-4 p-3">
                        <div className="d-flex justify-content-between align-items-center">
                            <div>
                                <h6 className="text-muted mb-2 fw-bold">Nombre de séances</h6>
                                <h2 className="text-dark-teal fw-bold mb-0">{totalSeances}</h2>
                            </div>
                            <div className="stat-icon-square bg-dark-teal text-white rounded-3 d-flex align-items-center justify-content-center">
                                <FontAwesomeIcon icon={faBook} />
                            </div>
                        </div>
                    </Card>
                </Col>
                <Col md={4}>
                    <Card className="border-0 shadow-sm rounded-4 p-3">
                        <div className="d-flex justify-content-between align-items-center">
                            <div>
                                <h6 className="text-muted mb-2 fw-bold">Places prévues</h6>
                                <h2 className="text-dark-teal fw-bold mb-0">{totalPlaces}</h2>
                            </div>
                            <div className="stat-icon-square bg-dark-teal text-white rounded-3 d-flex align-items-center justify-content-center">
                                <FontAwesomeIcon icon={faUser} />
                            </div>
                        </div>
                    </Card>
                </Col>
            </Row>

            {error && (
                <Alert variant="danger" className="mb-3">
                    {error}
                </Alert>
            )}

            {success && (
                <Alert variant="success" className="mb-3">
                    {success}
                </Alert>
            )}

            <Card className="border-0 shadow-sm rounded-4 overflow-hidden">
                <Card.Header className="bg-white p-4 border-0 d-flex justify-content-between align-items-center flex-wrap gap-3">
                    <h4 className="fw-bold text-dark-teal mb-0">
                        <div className="d-inline-flex align-items-center justify-content-center bg-dark-teal text-white rounded-2 me-3" style={{ width: 32, height: 32 }}>
                            <FontAwesomeIcon icon={faBook} size="xs" />
                        </div>
                        Liste des sessions
                    </h4>
                    <div className="d-flex gap-3">
                        <Button variant="dark-teal" className="rounded-2 px-3 fw-bold" onClick={handleShowAdd}>
                            Ajouter une session
                        </Button>
                    </div>
                </Card.Header>
                <Card.Body className="p-0">
                    <Table responsive hover className="mb-0 align-middle">
                        <thead className="bg-light">
                            <tr>
                                <th className="ps-4 py-3 text-muted border-0">Nom</th>
                                <th className="py-3 text-muted border-0">Professeur</th>
                                <th className="py-3 text-muted border-0 text-center">Séances</th>
                                <th className="py-3 text-muted border-0 text-center">Places</th>
                                <th className="py-3 text-muted border-0 text-center">Places Restantes</th>
                                <th className="py-3 text-muted border-0 text-center">Date début</th>
                                <th className="py-3 text-muted border-0 text-center">Date fin</th>
                                <th className="pe-4 py-3 text-muted border-0 text-end"></th>
                            </tr>
                        </thead>
                        <tbody>
                            {isLoading ? (
                                <tr>
                                    <td className="text-center py-4" colSpan={8}>
                                        <Spinner animation="border" role="status" />
                                    </td>
                                </tr>
                            ) : sessions.length === 0 ? (
                                <tr>
                                    <td className="text-center py-4" colSpan={8}>
                                        Aucune session pour cette formation.
                                    </td>
                                </tr>
                            ) : (
                                sessions.map((session) => (
                                    <tr key={session.id} className="border-bottom">
                                        <td className="ps-4 py-3 fw-bold text-dark">Session #{session.id}</td>
                                        <td className="py-3 text-muted">
                                            {session.intervenant ? `${session.intervenant.nom} ${session.intervenant.prenom}` : '—'}
                                        </td>
                                        <td className="py-3 text-muted text-center">{seancesBySession[session.id] || 0}</td>
                                        <td className="py-3 text-muted text-center">{session.nbPlace || '—'}</td>
                                        <td className="py-3 text-muted text-center">{session.nbPlaceRestant || '—'}</td>
                                        <td className="py-3 text-center">
                                            <Badge bg="primary" className="rounded-pill px-3 py-2 fw-normal opacity-75 text-white">
                                                {session.date_start || '—'}
                                            </Badge>
                                        </td>
                                        <td className="py-3 text-center">
                                            <Badge bg="success" className="rounded-pill px-3 py-2 fw-normal opacity-75 text-white">
                                                {session.date_end || '—'}
                                            </Badge>
                                        </td>
                                        <td className="pe-4 py-3 text-end">
                                            <div className="d-flex justify-content-end gap-3">
                                                <Button variant="link" className="p-0 text-muted" onClick={() => navigate(`/admin/sessions/${session.id}`)}>
                                                    <FontAwesomeIcon icon={faEye} />
                                                </Button>
                                                <Button variant="link" className="p-0 text-muted" onClick={() => handleShowEdit(session)}>
                                                    <FontAwesomeIcon icon={faPen} />
                                                </Button>
                                                <Button variant="link" className="p-0 text-danger" onClick={() => handleDeleteSession(session.id)}>
                                                    <FontAwesomeIcon icon={faTrash} />
                                                </Button>
                                            </div>
                                        </td>
                                    </tr>
                                ))
                            )}
                        </tbody>
                    </Table>
                </Card.Body>
            </Card>

            <Modal show={showAddModal} onHide={handleCloseAdd} centered size="lg">
                <Modal.Header closeButton>
                    <Modal.Title>Ajouter une session</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Form onSubmit={handleCreateSession}>
                        <Form.Group className="mb-4">
                            <Form.Label className="form-label-custom">Date de début <span className="text-red">*</span></Form.Label>
                            <Form.Control name="date_start" value={createForm.date_start} onChange={handleChange(setCreateForm)} type="date" className="form-control-custom" required />
                        </Form.Group>

                        <Form.Group className="mb-4">
                            <Form.Label className="form-label-custom">Date de fin <span className="text-red">*</span></Form.Label>
                            <Form.Control name="date_end" value={createForm.date_end} onChange={handleChange(setCreateForm)} type="date" className="form-control-custom" required />
                        </Form.Group>

                        <Form.Group className="mb-4">
                            <Form.Label className="form-label-custom">Professeur</Form.Label>
                            <Form.Select name="intervenantId" value={createForm.intervenantId} onChange={handleChange(setCreateForm)} className="form-control-custom">
                                <option value="">-- Sélectionner un professeur --</option>
                                {intervenants.map((intervenant) => (
                                    <option key={intervenant.id} value={intervenant.id}>
                                        {intervenant.nom} {intervenant.prenom}
                                    </option>
                                ))}
                            </Form.Select>
                        </Form.Group>

                        <Form.Group className="mb-5">
                            <Form.Label className="form-label-custom">Nombre de places</Form.Label>
                            <Form.Control name="nbPlace" value={createForm.nbPlace} onChange={handleChange(setCreateForm)} type="number" placeholder="30" className="form-control-custom" />
                        </Form.Group>

                        <div className="text-center">
                            <Button className="btn-modal-action" type="submit">
                                Ajouter
                            </Button>
                        </div>
                    </Form>
                </Modal.Body>
            </Modal>

            <Modal show={showEditModal} onHide={handleCloseEdit} centered size="lg">
                <Modal.Header closeButton>
                    <Modal.Title>Modifier la session</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Form onSubmit={handleUpdateSession}>
                        <Form.Group className="mb-4">
                            <Form.Label className="form-label-custom">Date de début <span className="text-red">*</span></Form.Label>
                            <Form.Control name="date_start" value={editForm.date_start} onChange={handleChange(setEditForm)} type="date" className="form-control-custom" required />
                        </Form.Group>

                        <Form.Group className="mb-4">
                            <Form.Label className="form-label-custom">Date de fin <span className="text-red">*</span></Form.Label>
                            <Form.Control name="date_end" value={editForm.date_end} onChange={handleChange(setEditForm)} type="date" className="form-control-custom" required />
                        </Form.Group>

                        <Form.Group className="mb-4">
                            <Form.Label className="form-label-custom">Professeur</Form.Label>
                            <Form.Select name="intervenantId" value={editForm.intervenantId} onChange={handleChange(setEditForm)} className="form-control-custom">
                                <option value="">-- Sélectionner un professeur --</option>
                                {intervenants.map((intervenant) => (
                                    <option key={intervenant.id} value={intervenant.id}>
                                        {intervenant.nom} {intervenant.prenom}
                                    </option>
                                ))}
                            </Form.Select>
                        </Form.Group>

                        <Form.Group className="mb-5">
                            <Form.Label className="form-label-custom">Nombre de places</Form.Label>
                            <Form.Control name="nbPlace" value={editForm.nbPlace} onChange={handleChange(setEditForm)} type="number" className="form-control-custom" />
                        </Form.Group>

                        <div className="text-center">
                            <Button className="btn-modal-action" type="submit">
                                Modifier
                            </Button>
                        </div>
                    </Form>
                </Modal.Body>
            </Modal>
        </Container>
    );
};

export default AdminFormationSessions;
