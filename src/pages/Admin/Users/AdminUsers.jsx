            ...
            <Row className="g-4 mb-5">
                ...
                <Col md={6}>
                    <Card className="border-0 shadow-sm rounded-4 p-3 ps-4">
                        <div className="d-flex justify-content-between align-items-center">
                            <div>
                                <h6 className="text-muted mb-2 fw-bold">Nombre de certificats délivrés</h6>
                                <h2 className="text-dark-teal fw-bold mb-0">240</h2>
                            </div>
                            <div className="stat-icon-square bg-dark-teal text-white rounded-3 d-flex align-items-center justify-content-center">
                    ...
                    <Table responsive hover className="mb-0 align-middle">
                        <thead className="bg-light">
                            <tr>
                                ...
                                <th className="py-3 text-muted border-0 text-center">Certifications</th>
                        ...
                        <tbody>
                            {adminUsers.map((user) => (
                                <tr key={user.id} className="border-bottom">
                                    ...
                                    <td className="py-3 text-muted text-decoration-underline text-dark">{user.email}</td>
                                    <td className="py-3 text-center">
                                        <Badge bg="primary" className="rounded-pill px-3 py-2 fw-normal opacity-50 text-blue-custom" style={{ minWidth: '40px', backgroundColor: '#7b96d4 !important', color: '#fff' }}>
                                            {user.formationsCount}
                                        </Badge>
                                    </td>
                                    <td className="py-3 text-center">
                                        <Badge bg="success" className="rounded-pill px-3 py-2 fw-normal opacity-50 text-green-custom" style={{ minWidth: '40px', backgroundColor: '#aaeebb !important', color: '#111' }}>
                                            {user.certificationsCount}
                                        </Badge>
                                    </td>
                                    <td className="py-3 text-muted">{user.inscriptionDate}</td>
            ...
            <Modal show={showEditModal} onHide={handleCloseEdit} centered size="lg">
                ...
                <Modal.Body>
                    <Form>
                        <Form.Group className="mb-4">
                            <Form.Label className="form-label-custom">Nom <span className="text-red">*</span></Form.Label>
                            <Form.Control type="text" defaultValue={selectedUser?.lastName} className="form-control-custom" />
                        </Form.Group>
                        ...
                        <Form.Group className="mb-5">
                            <Form.Label className="form-label-custom">E-mail <span className="text-red">*</span></Form.Label>
                            <Form.Control type="email" defaultValue={selectedUser?.email} className="form-control-custom" />
                        </Form.Group>
